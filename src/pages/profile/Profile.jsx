...
        "http://3.39.56.40:8080/api/points", 
        {
          headers: {
...
  	  alert("충전이 완료되었습니다.");
  	  
  	  fetchPoints(); // 포인트 새로고침
  	  fetchLocalCurrency(); // 지역 화폐 새로고침
...
  	try {
  	  // [수정] 알려주신 POST와 URL(/api/coupons/redeem)로 변경
  	  const response = await axios.post(
  	 	 "http://3.39.56.40:8080/api/coupons/redeem", // <-- 여기 수정됨
  	 	 { 
  	 	   // API 명세서 Body
...
  	  
  	  // 성공 응답 처리 (명세서 기준)
  	  alert(response.data.message || "성공적으로 교환되었습니다."); // 성공 메시지
  	  setPoints(response.data.remainPoint); // [중요] 남은 포인트 새로고침
  	  
  	  // (선택) 교환 성공 시, 보유 쿠폰 목록(모달 5)을 다시 불러올 수 있습니다.
  	  // handleShowOwnedCoupons(); 

  	} catch (err) {
  	  console.error("쿠폰 교환 실패:", err);
  	  // 에러 처리
  	  if (err.response) {
  	 	 // 400은 "포인트 부족"일 가능성이 높습니다.
  	 	 if (err.response.status === 400) {
  	 	   alert("포인트가 부족합니다. (서버)");
  	 	 } else if (err.response.status === 401 || err.response.status === 404) {
  	 	   alert("인증 정보가 올바르지 않습니다.");
...
  	 	 	 		   onClick={(e) => {
  	 	 	 			 e.stopPropagation();
  	 	 	 			 setSelectedCoupon(coupon); // [추가] 어떤 쿠폰을 눌렀는지 기억
...
  	 	   onClick={() => {
  	 	 	  setShowExchangeModal(false);
  	 	 	  setSelectedCoupon(null); // [추가] 선택 해제
  	 	   }}
  	 	   >
  	 	   <div
  	 	 	 style={{
...
  	 	 	 {/* [수정] 선택된 쿠폰 이름 동적 표시 */}
  	 	 	 <div style={{ fontSize: 18, fontWeight: 700, textAlign: 'center', marginBottom: 24, marginTop: 12 }}>
  	 	 	   {selectedCoupon?.name || "쿠폰"}으로<br/>
  	 	 	   교환하시겠습니까?
  	 	 	 </div>
  	 	 	 
  	 	 	 <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
  	 	 	 <button
  	 	 	   onClick={handleExchangeCoupon} // [수정] API 핸들러 연결
  	 	 	   style={{
  	 	 	   background: '#5E936C',
  	 	 	   color: '#fff',
  	 	 	   border: 'none',
